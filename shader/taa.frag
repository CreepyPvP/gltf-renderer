...
void main() 
{
    ...
    vec2 uv_corrected = uv - jitter_correction;
    ...
    vec2 vel = vec2(0);
    // vec2 vel = texture(velocity, uv_jittered).xy;
    ...
    vec2 prev_uv = uv_corrected - vel;

    vec3 source_sample = texture(current_frame, uv_curr + sample_offset).rgb;
